...
# We won't be testing this in the Tutor, but it will help you in debugging on your own machine.
# It should return a tuple of theta (a d by 1 array) and theta_0 (a 1 by 1 array).


# Your function should initialize all parameters to 0, then run through the data, in the order it is given, 
# performing an update to the parameters whenever the current parameters would make a mistake on that data point.
# Perform T iterations through the data. After every parameter update, if hook is defined,
# call it on the current (th, th0) (as a single parameter in a python tuple).

# When debugging on your own, you can use the procedure test_linear_classifier for testing. 
# By default, it pauses after every parameter update to show the separator. 
# For data sets not in 2D, or just to get the answer, set draw = False.
...
# Regular perceptron can be somewhat sensitive to the most recent examples that it sees. 
# Instead, averaged perceptron produces a more stable output by outputting the average value of th and th0 across all iterations.
...
# To evaluate a classifier, we are interested in how well it performs on data that it wasn't trained on.
# Construct a testing procedure that uses a training data set, calls a learning algorithm to get a linear separator (a tuple of theta, theta_0), 
# and then reports the percentage correct on a new testing set as a float between 0. and 1..

# The learning algorithm is passed as a function that takes a data array and a labels vector.
# Your evaluator should be able to interchangeably evaluate perceptron or averaged_perceptron 
# (or future algorithms with the same spec), depending on what is passed through the learner parameter.
...
def eval_classifier(learner, data_train, labels_train, data_test, labels_test):
    ...
    n_test_data = data_test.shape[1]
    tht, tht0 = learner(data_train, labels_train)
    correct = score(data_test, labels_test, tht, tht0)
    return correct/n_test_data
...
# Construct a testing procedure that takes a learning algorithm and a data source as input and runs 
# the learning algorithm multiple times, each time evaluating the resulting classifier as above.
# It should report the overall average classification accuracy.
...
# Note: Be sure to generate your training data and then testing data in that order,
# to ensure that the pseudorandomly generated data matches that in the test code.
...
def eval_learning_alg(learner, data_gen, n_train, n_test, it):
    ...
    for _ in range(it):
        ...
        tht, tht0 = learner(train_data, train_labels)
        correct = score(test_data, test_labels, tht, tht0)
        total_accuracy += correct/n_test
    return total_accuracy/it
...
# Cross-validation is a strategy for evaluating a learning algorithm, using a single training set of size n.
# Cross-validation takes in a learning algorithm L, a fixed data set D, and a parameter k.
# It will run the learning algorithm k different times, then evaluate the accuracy of the resulting classifier,
# and ultimately return the average of the accuracies over each of the k "runs" of L. 
...
# So, each time, it trains on k−1 of the pieces of the data set and tests 
# the resulting hypothesis on the piece that was not used for training.

# When k=n, it is called leave-one-out cross validation.

# Implement cross validation assuming that the input data is shuffled already so that the
# positives and negatives are distributed randomly. If the size of the data does not evenly 
# divide by k, split the data into n % k sub-arrays of size n//k + 1 and the rest of size n//k. 

# (Hint: You can use numpy.array_split and numpy.concatenate with axis arguments to split and rejoin the data as you desire.)

# Note: In Python, n//k indicates integer division, e.g. 2//3 gives 0 and 4//3 gives 1.

def xval_learning_alg(learner, data, labels, k):
    d, n = data.shape
    data_partitions = np.array_split(data, k, axis=1)
    label_partitions = np.array_split(labels, k, axis=1)
    accuracy_sum = 0
    for i in range(k):
        if i == 0:
            train_labels = np.concatenate(label_partitions[1:], axis=1)
            train_data = np.concatenate(data_partitions[1:], axis=1)
            test_labels = label_partitions[i]
            test_data = data_partitions[i]
            ...
            train_labels = np.concatenate(label_partitions[0:i], axis=1)
            train_data = np.concatenate(data_partitions[0:i], axis=1)
            test_labels = label_partitions[i]
            test_data = data_partitions[i]
            _, n_test = test_data.shape
            tht, tht0 = learner(train_data, train_labels)
            ...
            left_partition_data = np.concatenate(data_partitions[:i], axis=1)
            right_partition_data = np.concatenate(data_partitions[i+1:], axis=1)
            left_partition_labels = np.concatenate(label_partitions[:i], axis=1)
            right_partition_labels = np.concatenate(label_partitions[i+1:], axis=1)
            train_labels = np.concatenate((left_partition_labels, right_partition_labels), axis=1)
            train_data = np.concatenate((left_partition_data, right_partition_data), axis=1)
            test_labels = label_partitions[i]
            test_data = data_partitions[i]
            _, n_test = test_data.shape
            tht, tht0 = learner(train_data, train_labels)
...
# In this section, we compare the effectiveness of perceptron and 
# averaged perceptron on some data that are not necessarily linearly separable.

# Use your eval_learning_alg and the gen_flipped_lin_separable function in the code file 
# to evaluate the accuracy of perceptron vs. averaged_perceptron. gen_flipped_lin_separable 
# is a wrapper function that returns a generator - flip_generator, which can be called with an 
# integer to return a data set and labels. Note that this generates linearly separable data and then 
# "flips" the labels with some specified probability (the argument pflip); so most of the results 
# will not be linearly separable. You can also specifiy pflip in the call to the 
# generator wrapper function. At the bottom of the code distribution is an example.

# Run enough trials so that you can confidently predict the accuracy of these algorithms on 
# new data from that same generator; assume training/test sets on the order of 20 points. 
# The Tutor will check that your answer is within 0.025 of the answer we got using the same generator.

def modified_eval_learning_alg(learner, data_gen, n_train, it):
    total_accuracy = 0
    for _ in range(it):
        train_data, train_labels = data_gen(n_train)
        tht, tht0 = learner(train_data, train_labels)
        correct = score(train_data, train_labels, tht, tht0)
        total_accuracy += correct/n_train
    return total_accuracy/it
...
print(modified_eval_learning_alg(perceptron, gen_flipped_lin_separable(pflip=.1), 20, 5))

print(modified_eval_learning_alg(averaged_perceptron, gen_flipped_lin_separable(pflip=.1), 20, 5))

print(modified_eval_learning_alg(perceptron, gen_flipped_lin_separable(pflip=.25), 20, 5))

print(modified_eval_learning_alg(averaged_perceptron, gen_flipped_lin_separable(pflip=.25), 20, 5))
